...
namespace jsvk
{
	...
	static ResourcesXRVS::TypeCmd s_type_resources_vs;

	ResourcesXRVS::ResourcesXRVS()
	{
	}

	void ResourcesXRVS::createFramebuffer()
	{
	...
	void ResourcesXRVS::hotReloadPipeline()
	{
	...
	int ResourcesXRVS::loadModel(std::vector<std::string> modelPaths)
	{
	...
	void ResourcesXRVS::init(jsvk::VulkanDevice *pVulkanDevice, jsk::Presenter *presenter, VkSampleCountFlagBits msaaSamples, int width, int height)
	{
	...
	void ResourcesXRVS::createPipeline()
	{
		VkFormat depthFormat = jsvk::findDepthFormat(m_pVulkanDevice->m_pPhysicalDevice);

		jsvk::createImageOnly(m_pVulkanDevice->Device(), m_extent.width, m_extent.height, 1, VK_SAMPLE_COUNT_1_BIT, depthFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_DEPTH_STENCIL_ATTACHMENT_BIT, m_renderPasses[0].depth.image);
		...
		jsvk::createImageOnly(m_pVulkanDevice->Device(), m_extent.width, m_extent.height, 1, VK_SAMPLE_COUNT_1_BIT, colorFormat, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSIENT_ATTACHMENT_BIT | VK_IMAGE_USAGE_COLOR_ATTACHMENT_BIT, m_renderPasses[0].color.image);
	...
	void ResourcesXRVS::createRenderPass()
	{
