...
namespace jsvk
{
	class VertRenderer : public Renderer
	{
	public:
		class TypeCmd : Renderer::RenderType
		{
			bool isAvailable() const { return 1; }
			const char *name() const { return "Vertex Renderer"; }
			Renderer *create() const
			{
				VertRenderer *renderer = new VertRenderer();
				return renderer;
			}
	...
	static VertRenderer::TypeCmd s_type_cmdbuffer_vk;

	bool VertRenderer::init(jsvk::VulkanDevice *pVulkanDevice, jsvk::Resources *pResources)
	{
	...
	void VertRenderer::updateUniforms()
	{
	...
	void VertRenderer::drawDynamic(GameObject *mesh, uint32_t imageIndex, VkCommandBuffer *primaryBuffer, VkCommandBufferInheritanceInfo &inheritInfo, GameObject *offsetMesh, int gameObjectOffset)
	{
	}

	void VertRenderer::draw(uint32_t imageIndex, VkCommandBuffer *primaryBuffer)
	{
		std::array<VkClearValue, 2> clearValues = {};
		clearValues[0].color = {0.392f, 0.584f, 0.929f, 1.0f};
		clearValues[1].depthStencil = {1.0f, 0};
